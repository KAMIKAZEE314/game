...
use crate::core::player::{self, Player};
...
fn main() {
    ...
    let mut player = core::player::Player::new("erneesto".to_string(), ATTACKS.to_vec(), vec![], 0, 100, 1, Some(floor), 0, 0); 

    println!("{:#?}", player);
    player.save_player("data.txt");
    */
    let player = Player::load_player("data.txt");
    println!("{:#?}", player);
    /*
    let ATTACKS: [core::player::Attack; 1] = [core::player::Attack::new("sudo rm -fr /", 99999, 3, 1, 1)];
    let mut rng = rand::thread_rng();
    ...
    let mut player = core::player::Player::new("erneesto", ATTACKS.to_vec(), vec![], 0, 100, 1, None, 0, 0);
    
    player.set_floor(Some(floor));
    println!("{:#?}", player);
    ...
    let current_floor: &mut core::Floor = match player.get_mut_current_floor() {
        Some(floor) => floor,
        None => panic!("No floor available!"),
    };


    let attack_targets = current_floor.get_mut_enemys();
